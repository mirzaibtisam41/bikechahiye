...
const PostDetail = ({ productBlog, blog }) => {
  ...
  const validateContent = (content, id) => {
    ...
    content = content
      .split(`[/et_pb_column][/et_pb_row][/et_pb_section`)
      .join("");
  ...
  const omitStartHtml = (c) => {
    ...
    if (
      c.includes("admin_label=&#8221;Text&#8221;]") &&
      c.includes("If you are here")
    ) {
      c = c.substring(
        c.indexOf("admin_label=&#8221;Text&#8221;]") + 30,
        c.length
      );
      c = c.split("strong").join("span");
      c = c
        .split(`<span style="text-decoration: underline;">`)
        .join('<span style="text-decoration: none;">');
  ...
  const blackHeadings = (content) => {
    ...
    content = content
      .split(`<span style="color: var(--tcb-skin-color-0);"`)
      .join(`<span style="color:black"`);
    content = content
      .split(`<a href`)
      .join(`<a style="text-decoration:none" href`);
  ...
  const validateConclusion = (c, id) => {
    var content = c.substring(
      0,
      c.indexOf("[/et_pb_text][/et_pb_column][/et_pb_row][/et_pb_section]")
    );
    return content;
  };
    ...
    <div style={{ backgroundColor: "white" }}>
      ...
      >
        <p className={classes.tit}>
          {" "}
          {ReactHtmlParser(productBlog && productBlog.title)}
        </p>
      </div>
                  ...
                  <div style={Styles.tableSubHeadings}>
                    {item.Subheadings.map((subheading, indexify) => {
                      return (
                        ...
                        >
                          {ReactHtmlParser(
                            validateTableOfContetSubheading(subheading)
                          )}
                        </div>
                      );
                    })}
      ...
      {productBlog?.sections?.map((item, index) => {
        return (
          <div key={index}>
            ...
            >
              {" "}
              {ReactHtmlParser(validateContentHeading(item.Heading))}{" "}
            </h2>

            <div
              id={item.subheading && item.subheading}
              style={{
                fontFamily: "Roboto",
                fontSize: "20px",
                fontStyle: "normal",
      ...
      <div>
        {ReactHtmlParser(
          productBlog.conclusion && validateConclusion(productBlog.conclusion)
        )}
      </div>
    </div>
  );
